...
def emit_hypothesis_table(machine, machine_idx, results, path: Path, seed: int) -> None:
    """Write the per-machine hypothesis-test table (full-width float)."""
    rows = []
    for r in results:
        ci = f"$[{r.ci_low:+.2f}, {r.ci_high:+.2f}]$"
        decision = r"\textbf{reject}" if r.reject else r"retain"
        # Format medians: use scientific notation for large values
        med_a2l = f"${r.median_a2l:.2e}$" if r.median_a2l > 9999 else f"${r.median_a2l:.1f}$"
        med_htlc = f"${r.median_htlc:.2e}$" if r.median_htlc > 9999 else f"${r.median_htlc:.1f}$"
        rows.append(
            f"    {r.hypothesis} & {DV_LABEL[r.variable]} "
            f"& {med_a2l} & {med_htlc} "
    ...
    path.write_text(
        ...
        "\\begin{table*}[!htbp]\n"
        "  \\centering\n"
        f"  \\caption{{Hypothesis tests (M{machine_idx}).}}\n"
        f"  \\label{{tab:hyp-m{machine_idx}}}\n"
        "  \\resizebox{\\textwidth}{!}{%\n"
        "  \\begin{tabular}{@{}l l r r r r l l@{}}\n"
        "    \\toprule\n"
        "    H & Variable & A$^2$L & HTLC"
        " & $p$ & $\\delta$ & 95\\% CI & Decision \\\\\n"
        "    \\midrule\n"
        f"{body}\n"
        "    \\bottomrule\n"
        "  \\end{tabular}}\n"
        f"  \\vspace{{-2mm}}\n"
        f"  {{\\scriptsize Mann--Whitney $U$, $\\alpha'\\approx{ALPHA_PRIME:.4f}$,"
        f" Cliff's $\\delta$ with BCa 95\\% CI.}}\n"
        "\\end{table*}\n"
    )
...
def emit_descriptives_table(per_run: pd.DataFrame, machines, path: Path) -> None:
    ...
    for mi, machine in enumerate(machines):
        for arm in ("a2l", "htlc"):
            ...
            # Use short machine label: M1, M2, etc.
            machine_label = f"M{mi + 1}"
            rows.append(
                f"    {machine_label} & {arm_label} "
    ...
    body = "\n".join(rows)
    # Build machine legend (escape underscores for LaTeX)
    machine_legend = ", ".join(f"M{i+1}: {m.replace('_', r'\_')}" for i, m in enumerate(machines))
    path.write_text(
        "% generated by analysis/run.py -- do not edit\n"
        "\\begin{table}[t]\n"
        "  \\centering\\small\n"
...
def main():
    ...
    for mi, machine in enumerate(machines):
        ...
        # Use M1, M2, etc. for filenames and labels
        emit_hypothesis_table(
            machine, mi + 1, machine_results,
            args.tables_dir / f"hypotheses_m{mi + 1}.tex",
            args.seed,
        )
